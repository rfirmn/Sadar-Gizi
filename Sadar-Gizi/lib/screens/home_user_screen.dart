...
class _HomeUserScreenState extends State<HomeUserScreen> {
  ...
  final double konsumsiGula = 36;
  final double targetGula = 50;

  final List<double> trenGulaMingguan = [40, 50, 55, 60, 45, 70, 65];
  ...
  void initState() {
    ...
    // Ambil produk yang dikonsumsi hari ini setelah widget selesai build
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final uid = FirebaseAuth.instance.currentUser!.uid;
      Provider.of<ConsumedProductProvider>(context, listen: false)
          .fetchTodayConsumedProducts(uid);
    });
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Consumer<ConsumedProductProvider>(
                builder: (context, provider, _) {
                  final todayProducts = provider.consumedProducts
                      .where((p) => p.consumedDate != null &&
                          p.consumedDate!.day == DateTime.now().day &&
                          p.consumedDate!.month == DateTime.now().month &&
                          p.consumedDate!.year == DateTime.now().year)
                      .toList();

                  if (todayProducts.isEmpty) {
                    return const Text("Belum ada konsumsi hari ini.");
                  }

                  return Column(
                    children: todayProducts.map((product) {
                      return Container(
                            ...
                            Expanded(
                              child: Padding(
                                ...
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    // Hapus tanggal, langsung tampilkan NutriGrade
                                    CircleAvatar(
                                      radius: 12,
